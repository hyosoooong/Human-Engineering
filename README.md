...
Type A 와 Type B 는 **동일한 8개 지문**을 공유하며, 조건 간 차이는 출력 방식뿐입니다. `stimuli.js` 의 단일 `STIMULI` 배열을 편집하면 되고, `experiment.js` 는 `getStimuliForCondition(condition)` 으로 (조건과 무관하게) 같은 배열을 불러옵니다. 자극을 8개 외의 개수로 변경하면 `experiment.js` 의 전송 ID 목록(`buildPayload` 내 `['S1'..'N4']`)과 `google-apps-script.gs` 의 `HEADERS` 도 함께 수정해야 합니다.

### 자극 ID ↔ 내용 매핑

4개 도메인(법률·투자·의료·정보보호)에서 정상/오류 지문을 추출해 **정상 4 + 오류 4**(총 8개)로 구성했습니다. Type A·B 모두 아래 8개를 동일하게 사용합니다.

| ID | 내용 | 정답 |
|---|---|---|
| `N1` | 법률 — 노트북 계약 해제(정상) | 활용함 |
| `N2` | 정보보호 — 공용 와이파이(정상) | 활용함 |
| `N3` | 투자 — B푸드(정상) | 활용함 |
| `N4` | 의료 — 감기약 병용(정상) | 활용함 |
| `S1` | 투자 — A테크(언어표현 불일치) | 활용 안 함 |
| `S2` | 의료 — 알레르기 응급(인과관계 오류) | 활용 안 함 |
| `S3` | 정보보호 — 비밀번호 점검(수치 불일치) | 활용 안 함 |
| `S4` | 법률 — 보증금 반환(개수 불일치) | 활용 안 함 |

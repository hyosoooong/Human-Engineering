...
| `nfc_1` ~ `nfc_15` | NFC 15문항 (각 1~5). 출처: 단축형 인지욕구척도 K-NfC-S(김완석, 2007). **역채점(부정문) 항목: 2·4·10·11번** → 분석 시 해당 4문항만 `6 − 응답값`으로 환산한 뒤 합산/평균 |
| `stimulus_order` | 제시 순서, 예: `S2,N1,S3,N3,S1,N2`. `S1~S3`=오류 자극, `N1~N3`=정상 자극. **같은 ID라도 조건(A/B)에 따라 내용이 다름** → `ui_condition`과 함께 해석 |
...
자극은 조건별로 분리되어 있습니다. `stimuli.js` 의 `STIMULI_A`(Type A) / `STIMULI_B`(Type B) 배열을 편집하면 되고, `experiment.js` 는 `getStimuliForCondition(condition)` 으로 알맞은 배열을 불러옵니다. 정보보호 자극 2개(`SEC_OK`=N3·정상, `SEC_ERR`=S3·오류)는 두 조건이 공유합니다. 자극을 6개 외의 개수로 변경하면 `google-apps-script.gs` 의 `HEADERS` 도 함께 수정해야 합니다.

### 자극 ID ↔ 내용 매핑

5개 도메인(법률·투자·직무·의료·정보보호)에서 정상/오류 지문을 추출해, 각 조건당 **정상 3 + 오류 3**으로 구성했습니다. 5가지 오류 유형(개수·언어표현·수치·인과관계·상황 불일치)이 실험 전체에 모두 포함됩니다.

| ID | Type A 내용 | 정답 | Type B 내용 | 정답 |
|---|---|---|---|---|
| `N1` | 법률 — 노트북 계약 해제(정상) | 활용함 | 투자 — B푸드(정상) | 활용함 |
| `N2` | 직무 — 서비스 운영(정상) | 활용함 | 의료 — 감기약 병용(정상) | 활용함 |
| `N3` | 정보보호 — 공용 와이파이(정상)·공유 | 활용함 | 정보보호 — 공용 와이파이(정상)·공유 | 활용함 |
| `S1` | 투자 — A테크(언어표현 불일치) | 활용 안 함 | 법률 — 보증금 반환(개수 불일치) | 활용 안 함 |
| `S2` | 의료 — 알레르기 응급(인과관계 오류) | 활용 안 함 | 직무 — 마케팅(상황 불일치) | 활용 안 함 |
| `S3` | 정보보호 — 비밀번호 점검(수치 불일치)·공유 | 활용 안 함 | 정보보호 — 비밀번호 점검(수치 불일치)·공유 | 활용 안 함 |
